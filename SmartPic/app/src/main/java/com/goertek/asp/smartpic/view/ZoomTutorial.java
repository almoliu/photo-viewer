...
public class ZoomTutorial {
    ...
    private static final String TAG = ZoomTutorial.class.getSimpleName();
  ...
  //  private ViewGroup mThumbViewParent;//小图片的视图
    ...
    private PicItem[] mPicItems;


    public ZoomTutorial(View containerView,View expandedView,PicItem[] picItems) {
        mContainView = containerView;
        mExpandedView = expandedView;
        View view = containerView.findViewById(R.id.recycler_pic_list);
        mPicItems = picItems;
        if(view instanceof RecyclerView)
            mRecyView = (RecyclerView)view;
        else
    ...
    public void zoomImageFromThumb(final View thumbView) {
        //mThumbViewParent = (ViewGroup) thumbView.getParent().getParent();
        //mRecyView  = (ViewGroup)mThumbViewParent.getParent().getParent();

        /*
        if(thumbView.getParent() instanceof FrameLayout) {
            Log.d(TAG,"***FrameLayout***");
        }

        if(thumbView.getParent().getParent() instanceof GridView) {
            Log.d(TAG,"***GridView***");
        }
        */
        ...
        mExpandedView.setBackgroundColor(Color.BLACK);
    ...
    public boolean getScaleFinalBounds(int position) {
        Log.d(TAG,"getScaleFinalBounds:"+position);
        //得到显示区域中第一个子视图的序号
        //Log.d(TAG,"count is:"+mThumbViewParent.getChildCount());

        int gridPos = getRecyChildPos(position);
        ...
        Log.d(TAG,"firstPosition:"+firstPosition+"gridpos:"+gridPos);
        View childView = mGridView.getChildAt(gridPos - firstPosition);
    ...
    private int getRecyChildPos(int pos) {
        int position = pos;

        for(int i = 0;i<mPicItems.length;i++) {
            Log.d(TAG,"recycler view count:"+mRecyView.getAdapter().getItemCount());

            Log.d(TAG,"getRecychildPost:pos"+pos+"child count:"+mPicItems[i].getItemNum());
            if(position<mPicItems[i].getItemNum()&&position<8) {
                PicGroupViewHolder picGroupViewHolder = (PicGroupViewHolder)mRecyView
                        .findViewHolderForAdapterPosition(i);
                if(picGroupViewHolder==null)
                    return -1;
                mGridView =  picGroupViewHolder.getGridView();
                Log.d(TAG,"return postion is:"+position);
                return position;
            }else {
                position = position- mPicItems[i].getItemNum();
            }
        }
        return position;
    }
    ...
    public void closeZoomAnim(int position) {
        ...
        mExpandedView.setBackgroundColor(Color.TRANSPARENT);
        set.start();
        mCurrentAnimator = set;
    }
